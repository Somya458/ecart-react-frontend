...
class ShowUserFoods extends Component {
    ...
    componentDidMount(){
        axios.get("http://localhost:8006/orders/get-all-food-items")
        .then((resp)=>{
    ...
    searchingFood = (e) =>{
        axios.post("http://localhost:8006/orders/search-by-fooditem", {"search" : document.getElementById('searchbyfood').value})
        .then((resp)=>{
                            ...
                            return(
                                <div className="restaurantsf" key={value.restaurantid+"-"+value.foodItem.fooditemid} id={index}>
                                    <div id="fooddata">
                                       ...
                                       <p className='UFLRating'>Rating : {value.foodItem.fooditemrating != null ? (Number(value.foodItem.fooditemrating).toPrecision(2) + " / 5") : "Rating not available"}</p>
                                    </div>                      
