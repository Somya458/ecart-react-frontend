...
export class Login extends Component {
  ...
  checkLogin=(e)=>
  {
      ...
      axios.post("http://localhost:8002/users/login",{phonenumber:phonenum,password:password})
      .then((res)=>{
        if(res.data==="phone")
        {
          document.getElementById("loginfalse1").innerHTML="Invalid phone number"
        }
        else if(res.data==="success_admin" || res.data==="Admin Logged_in Successfully!")
        {
          History.push({
            pathname:"/Admin",
          })
        ...
        else if(res.data==="success_user" || res.data==="User Logged_in Successfully!")
        {
          History.push({
            pathname:"/User",
            state:{
